...
// ? Variables
$id = $_SESSION['id'];
$target_dir = "../../../ppic/"; // Server directory
$file_name = $_FILES["uploadppic"]["name"]; // Client name
$file_tmpname = $_FILES["uploadppic"]["tmp_name"]; // Temp file name
...
// ? Check file size
if ($_FILES["uploadppic"]["size"] > 2000000) {
...
if ($uploadOk == 0) {
    ...
    if (!empty(glob($check_file))) {
        ...
        delete_ppic($_SESSION['id']);

        // * Deletes image(s) in the local folder

        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT name_ppic FROM user_ppic WHERE id_user= '" . $_SESSION['id'] . "'";
        $res = $pdo->query($sql);
    ...
    if (move_uploaded_file($_FILES["uploadppic"]["tmp_name"], $target_file)) {

        // ? And a new DB line
        insert_ppic($_SESSION['id'], $file_new_name, $uploadOk);
        echo "Le fichier " . htmlspecialchars(basename($_FILES["uploadppic"]["name"])) . " a bien été téléchargé.";
